...
class EsmScoring:
    def __init__(self):
        self.msa_transformer, self.msa_alphabet = \
            esm.pretrained.esm_msa1b_t12_100M_UR50S()
        self.layers_num = self.msa_transformer.num_layers
    ...
    def score(self, input_msa: parsers.Msa, max_num_msas: int):
        ...
        msa_transformer = self.msa_transformer.cuda().eval()
        with torch.no_grad():
    ...
    def score_sequences(
        ...
        def _score_cur_block():
            ...
            for chain_id, chain_msa in chain_msas.items():
                scores_ = self.score(chain_msa, max_num_msas)
                rows_ = rows[chain_id]
